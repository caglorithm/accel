...
*accel* is a work in process of building a low-cost sleep tracker. It can track your 💤 sleep during night through an accelerometer and detect how much you are moving. Using this information it then determines whether the user is in deep sleep (NREM stage II-III) and, if so, triggers the second stage: the acoustic brain stimulation protocol 🔊. This is a low-frequency acoustic stimulus 〰️ that is then delivered to the 🧠 brain via 🎧 headphones. 

Why trigger the stimulus in deep sleep? It is known that slow wave activity (~1Hz) in the sleeping human cortex is critical for memory consolidation, the process that writes memory from short-term memory to long-term memory. It has been repeatedly shown [*Marshall et al, Boosting slow oscillations during sleep potentiates memory. Nature (2006)*] that electrical stimulation of the brain in deep sleep stage can enhance memory consolidation. There is also evidence that acoustic stimulation through the auditory cortex can improve memory consolidation as well [*Leminen et al. Enhanced memory consolidation via automatic sound stimulation during non-REM sleep. Sleep (2017).*]. 
